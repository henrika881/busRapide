<header class="bg-white border-b border-slate-100 sticky top-0 z-40 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-6">
            <a href="/"
                class="text-sm font-bold {{ request()->is('/') ? 'text-brand-600 border-b-2 border-brand-600 pb-1' : 'text-slate-500 hover:text-brand-600' }} transition-colors">Accueil</a>
            <a href="/#search"
                class="text-sm font-bold text-slate-500 hover:text-brand-600 transition-colors">Rechercher</a>
            <a href="/#destinations"
                class="text-sm font-bold text-slate-500 hover:text-brand-600 transition-colors">Destinations</a>
            <a href="/#howto" class="text-sm font-bold text-slate-500 hover:text-brand-600 transition-colors">Comment ça
                marche</a>
            <a href="/#assistance"
                class="text-sm font-bold text-slate-500 hover:text-brand-600 transition-colors">Assistance</a>
        </nav>

        <!-- User Actions -->
        <div class="flex items-center gap-3">
            <script>
                // Immediate UI sync to prevent flickering
                (function () {
                    const token = localStorage.getItem('auth_token');
                    if (token) {
                        document.write('<style>#guest-buttons { display: none !important; } #mobile-guest-buttons { display: none !important; }</style>');
                    } else {
                        document.write('<style>#auth-user-display { display: none !important; } #mobile-auth-buttons { display: none !important; } #mobile-dashboard-link { display: none !important; } #mobile-tickets-link { display: none !important; }</style>');
                    }
                })();
            </script>

            <!-- User Account - Desktop (Hidden by default, shown via JS if auth) -->
            <div id="auth-user-display" class="hidden items-center gap-3">

                <a href="/profil" id="user-initial-display"
                    class="w-8 h-8 bg-brand-100 rounded-full flex items-center justify-center text-brand-600 font-bold shadow-sm hover:ring-2 hover:ring-brand-200 transition text-sm">
                    <span id="user-initial">U</span>
                </a>
                <button onclick="logoutUser()"
                    class="ml-2 w-8 h-8 bg-red-50 text-red-600 rounded-lg flex items-center justify-center hover:bg-red-100 transition-colors"
                    title="Déconnexion">
...
<!-- Mobile Menu -->
<div id="mobile-menu"
    class="mobile-menu fixed inset-0 z-50 bg-black/50 md:hidden opacity-0 pointer-events-none transition-opacity duration-300">
    <div class="absolute right-0 top-0 h-full w-64 bg-white shadow-xl transform transition-transform duration-300 translate-x-full"
        ...
        <div class="p-6">
            ...
            <div class="space-y-6">
                ...
                <a href="/profil" class="flex items-center gap-3 text-slate-700 font-bold text-lg hidden"
                    id="mobile-dashboard-link">
                    <i class="fa-solid fa-user-circle w-6"></i> Mon Profil
                </a>
                <a href="/tickets" class="flex items-center gap-3 text-slate-700 font-bold text-lg hidden"
                    id="mobile-tickets-link">
                    <i class="fa-solid fa-ticket w-6"></i> Mes Tickets
                </a>
            </div>

            <div class="absolute bottom-6 left-6 right-6 space-y-3" id="mobile-guest-buttons">
                <button onclick="openRegisterModal()"
                    class="w-full bg-white border border-brand-600 text-brand-600 py-3 rounded-xl font-bold">
                    S'inscrire
                </button>
